...
blog_server::blog_server(const std::string &config_file_path) {
  ...
  admin_user_manager__ = admin_user_manager::get_instance(
      sf_path_join(blog_config__.db_path, "admin_user.db3"s));
  ...
  blog_manager__ = blog_manager::get_instance(
      sf_path_join(blog_config__.db_path, "blogs.db3"s));
...
void blog_server::setup_server(const sf_http_server_config &server_conf) {
  ...
  admin_router->add_router(sf_http_router::make_instance(
      "/"s, function([this](const sf_http_request &req, sf_http_response &res) {
        admin_root(req, res);
      })));
...
void blog_server::admin_login(const sf_http_request &req,
  ...
  auto user_info =
      admin_user_manager__->check_user(param["name"], param["password"]);
